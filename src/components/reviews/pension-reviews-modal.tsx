		...
		for (const r of reviews) {
			...
			if (score >= 1 && score <= 5) {
				counts[score as keyof typeof counts] =
					(counts[score as keyof typeof counts] || 0) + 1;
			}
		}
		if (reviews.length === 0) {
			counts[5] = 4;
				...
				<div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-6">
					<div className="flex flex-col items-center justify-center rounded-2xl border border-primary/20 bg-primary/5 p-5 text-center">
						...
						<p className="text-[11px] text-muted-foreground mt-2 max-w-sm">
							Calificación destacada por su ambiente de estudio, excelente
							conexión a internet y trato hospitalario del anfitrión.
						</p>
					</div>
