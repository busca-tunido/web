...
export function PublishReviewModal({
	...
	const [stayDuration, setStayDuration] =
		useState<StayDurationCategory>("ONE_SEMESTER");
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const result = await submitPensionReview(
				pension.id,
				reviewPayload,
				token ?? undefined,
			);
			...
			if (
				...
				setErrorMessage(
					errStr || "Hubo un error al enviar tu reseña. Inténtalo nuevamente.",
				);
			}
		} finally {
				...
				<div className="overflow-y-auto flex-1 px-4 py-4 space-y-5">
					<DrawerHeader className="p-0 text-left">
						...
						<DrawerDescription className="text-xs text-muted-foreground mt-1">
							Tu opinión sincera orienta a otros universitarios y fomenta una
					...
					{successBanner && (
						<div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 flex items-center gap-3 text-emerald-600 animate-in fade-in">
							...
							<p className="text-xs font-semibold">
								¡Reseña publicada con éxito! Gracias por colaborar con la
								comunidad.
							</p>
						</div>
					)}
					...
					<section className="rounded-2xl border border-border bg-muted/20 p-4 text-center">
						...
						<p className="text-xs font-semibold text-amber-600 h-4">
							{RATING_LABELS[currentRatingValue] ||
												...
												<button
													...
													onClick={() =>
														setCleanlinessRating(
															cleanlinessRating === starVal ? null : starVal,
														)
													}
										...
										<div className="flex gap-1">
											{[1, 2, 3, 4, 5].map((starVal) => (
												<button
													...
													onClick={() =>
														setLandlordRating(
															landlordRating === starVal ? null : starVal,
														)
													}
													className="p-0.5 transition-transform hover:scale-110 active:scale-95 cursor-pointer"
										...
										<div className="flex gap-1">
											{[1, 2, 3, 4, 5].map((starVal) => (
												<button
													...
													onClick={() =>
														setQuietnessRating(
															quietnessRating === starVal ? null : starVal,
														)
													}
													className="p-0.5 transition-transform hover:scale-110 active:scale-95 cursor-pointer"
										...
										<div className="flex gap-1">
											{[1, 2, 3, 4, 5].map((starVal) => (
												<button
													...
													onClick={() =>
														setWifiRating(
															wifiRating === starVal ? null : starVal,
														)
													}
													className="p-0.5 transition-transform hover:scale-110 active:scale-95 cursor-pointer"
