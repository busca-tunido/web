...
import {
	NetworkErrorBanner,
	NetworkErrorState,
} from "@/components/common/network-error-state";
...
export function PensionDetailModal({
	...
	const hasAlreadyReviewed = Boolean(
		user && reviews.some((r) => r.user?.id === user.id),
	);
				...
				<DrawerContent className="max-h-[92vh] max-w-lg md:max-w-xl mx-auto md:rounded-3xl bg-card border-border p-6 flex flex-col items-center justify-center">
					<div className="flex flex-col items-center gap-3 py-16">
						...
						<p className="text-xs text-muted-foreground">
							Cargando detalles de la pensión...
						</p>
					</div>
				</DrawerContent>
			</Drawer>
				...
				>
					...
					<div className="overflow-y-auto flex-1 px-4 md:px-8 pt-2 pb-6">
							...
							<div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
								<div className="flex gap-1.5">
									{pension.photos.map((photoUrl, i) => (
										<button
											{/* biome-ignore lint/suspicious/noArrayIndexKey can't use other thing */}
						...
						<div className="mb-4 rounded-xl border border-primary/20 bg-primary/5 p-3">
							<div className="flex items-center gap-2 text-xs font-medium text-primary">
								...
								<span>
									A {pension.distanceToUniversityMeters}m de{" "}
									{pension.nearestUniversityName}
								</span>
							</div>
						</div>
					...
					<DrawerFooter className="border-t border-border/80 bg-card p-4 md:px-8">
						{hasAlreadyReviewed ? (
							...
							>
								<Check className="mr-2 h-4 w-4 text-primary" /> Ya calificaste
								esta pensión (Ver opiniones)
							</Button>
						) : (
							<Button
