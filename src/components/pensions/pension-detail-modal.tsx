...
export function PensionDetailModal({
  ...
  } = usePensionReviews(user ? targetId : null, {
  ...
  const pension = useMemo(() => {
    ...
    const effectiveAverage =
      ratingStats.average > 0 ? ratingStats.average : (activePension.ratingAverage ?? 0);
    const effectiveCount =
      ratingStats.count > 0 ? ratingStats.count : (activePension.reviewsCount ?? 0);

    return {
      ...activePension,
      ratingAverage: effectiveAverage,
      reviewsCount: effectiveCount,
    };
        ...
        >
          ...
          <div className="overflow-y-auto flex-1 px-4 md:px-8 pt-2 pb-6 pt-6">
            ...
            <div className="relative h-64 md:h-88 w-full rounded-2xl overflow-hidden bg-muted mb-4 shadow-sm">
              ...
              <div className="absolute top-3 right-3 flex items-center gap-2">
                <button
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-black/60 backdrop-blur-md text-white hover:text-primary active:scale-90 transition shadow-sm cursor-pointer"
              ...
              <div className="flex items-center gap-2 mb-1.5">
                ...
                >
                  ...
                  <span className="text-muted-foreground font-normal underline decoration-muted-foreground/40">
                    (
                    {pension.reviewsCount > 0
                      ? `${pension.reviewsCount} ${pension.reviewsCount === 1 ? 'reseña' : 'reseñas'}`
                      : 'Sin reseñas'}
                    )
                  </span>
                </button>
            ...
            <AuthGate
              ...
              description="Accede con tu cuenta institucional para conocer opiniones de estudiantes, normas de convivencia, ubicación exacta y disponibilidad de habitaciones."
              previewHeight="320px"
            >
              <div className="mb-5">
          ...
          <DrawerFooter className="border-t border-border/80 bg-card p-4 md:px-8">
            {user ? (
              ...
              <Button
                id="btn-auth-to-review"
                onClick={openAuthModal}
                className="w-full h-12 bg-primary hover:opacity-95 text-primary-foreground font-bold text-sm rounded-xl shadow-md active:scale-[0.98] transition cursor-pointer"
              >
                <PenLine className="mr-2 h-4 w-4" /> Inicia sesión para opinar o guardar
              </Button>
            )}
          </DrawerFooter>
        </DrawerContent>
